...
// computeRate computes a sliding-window average using the given samples.
// It assumes that the sample value at startTime is 0, which is useful for
// computing the initial rate.q
func computeRate(timestampsMillis, samples []int64, scale float64, startTime time.Time) []float64 {
	...
	for i := range samples {
		...
		for ; windowStartIdx < i; windowStartIdx++ {
			t, s := startTime.UnixMilli(), int64(0)
			if windowStartIdx >= 0 {
